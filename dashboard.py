...
from nlp_helpers import tokenizar, limpar, sem_stops, stemizar, binarizar, indice_de_palavras, vocab
from plotly import express as px
from examples import long_ds, long_da, long_de

st.title("Modelo de Classificação de Vagas")
descricao_vaga = None

inputs_possiveis = st.radio("Selecione a maneira que você quer dar o input do modelo",["Exemplos toy", "Exemplos reais do Linkedin", "Input customizado"])
if inputs_possiveis == "Exemplos toy":
    tipo_de_vaga = st.radio("Exemplos toy são descrições simples de cada função, criadas por mim sem nenhuma descrição de vaga como referência", ["", "Data Scientist", "Data Analyst", "Data Engineer"], format_func=lambda x: 'Selecione uma opção' if x == '' else x)
...
if inputs_possiveis == "Exemplos reais do Linkedin":
    tipo_de_vaga = st.radio("Estes foram exemplos que eu tirei diretamente do meu Linkedin e não estão nas bases usadas no modelo e nem tem o título da função na descrição da vaga", ["", "Data Scientist @ Volvo", "Data Analyst @ Dentsu International", "Data Engineer @ Amify"], format_func=lambda x: 'Selecione uma opção' if x == '' else x)
...
if inputs_possiveis == "Input customizado":
    descricao_vaga = st.text_area("Coloque abaixo a descrição de uma vaga de Data Scientist, Data Analyst ou Data Engineer. \n\nRecomendo copiar a descrição de uma vaga vinda do linkedin.", height=500)
...
if descricao_vaga:
    ...
    predicao = model.predict(np.array([vetores_msg_bin,]))
    predicao_dict = {
        0:"Data Scientist",
        1:"Data Analyst",
        2:"Data Engineer"
    ...
    fig = px.bar(df, "Profissão", "Probabilidade", text='Probabilidade', range_color=[0, 100], color="Probabilidade", color_continuous_scale='blues')
